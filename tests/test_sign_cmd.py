...
def test_sign_tx_jetton_transfer_non_expert(backend, navigator, test_name):
    import os
...
def test_sign_tx_jetton_transfer_non_expert_high_ton(backend, navigator, test_name):
    # Use the app interface instead of raw interface
    client = BoilerplateCommandSender(backend)
    # The path used for this entire test
    path: str = "m/44'/607'/0'/0'/0'/0'"
    ...
    # Create the transaction that will be sent to the device for signing
    tx = Transaction(Address(usdt_wallet_address), SendMode.PAY_GAS_SEPARATLY, 0, 1686176000, True, 1000000000,
                        payload=JettonTransferPayload(100, Address("0:" + "0" * 64), forward_amount=1, jetton_id=0))
    tx_bytes = tx.to_request_bytes()
